...
parser.add_argument('--model', type=str, default='./CDCL/weights/model_simulated_RGB_mgpu_scaling_append.0071.h5', help='path to the weights file')
...
parser.add_argument('--scale', action='append', help='<Required> Set flag', default='1')
...
import CDCL.util
from CDCL.config_reader import config_reader
from CDCL.model_simulated_RGB101 import get_testing_model_resnet101
from CDCL.human_seg.human_seg_gt import human_seg_combine_argmax
...
def get_steps(pta, ptb):
    d = math.sqrt((ptb[0]-pta[0])**2 + (ptb[1]-pta[1])**2)
    xstep = (ptb[0]-pta[0])/d * 3
    ystep = (ptb[1]-pta[1])/d * 3
    return [xstep, ystep]

def get_limb_control(zero, one, seg_canvas, seg_colour):
    if(zero[0] < 0 or zero[1] < 0 or one[0] < 0 or one[1] < 0):
        return []

    pts = np.zeros((8,2))

    q1 = zero + (one - zero) * 0.25
    q2 = zero + (one - zero) * 0.5
    q3 = zero + (one - zero) * 0.75
    steps = get_steps(zero, one)
    vsteps = [steps[1], -steps[0]]
    # print(seg_canvas[int(zero[0]), int(zero[1])])
    # print(seg_canvas[int(one[0]), int(one[1])])
    # print(seg_colour)
    if((seg_canvas[int(zero[0]), int(zero[1])]==seg_colour).all() and (seg_canvas[int(one[0]), int(one[1])]==seg_colour).all()):
        pt = zero
        while((seg_canvas[int(pt[0]), int(pt[1])] == seg_colour).all()):
            new_pt = pt-steps
            if(new_pt[0]>=0 and new_pt[1]>=0 and new_pt[0]<seg_canvas.shape[0] and new_pt[1]<seg_canvas.shape[1]):
                pt=new_pt
            else:
                break
        pts[0] = pt
    ...
    if((seg_canvas[int(zero[0]), int(zero[1])]==seg_colour).all() and (seg_canvas[int(one[0]), int(one[1])]!=seg_colour).all()):
        ...
        pts[0] = pt
        pt = one
        while(not (seg_canvas[int(pt[0]), int(pt[1])] == seg_colour).all()):
            new_pt = pt-steps
            if(new_pt[0]>=0 and new_pt[1]>=0 and new_pt[0]<seg_canvas.shape[0] and new_pt[1]<seg_canvas.shape[1]):
                pt=new_pt
            else:
    ...
    if((seg_canvas[int(q1[0]), int(q1[1])]==seg_colour).all()):
        pt = q1.copy()
        while((seg_canvas[int(pt[0]), int(pt[1])] == seg_colour).all()):
            pt+=vsteps
        pts[2] = pt
    ...
    if((seg_canvas[int(q2[0]), int(q2[1])]==seg_colour).all()):
        pt = q2.copy()
        while((seg_canvas[int(pt[0]), int(pt[1])] == seg_colour).all()):
            pt+=vsteps
        pts[4] = pt
    ...
    if((seg_canvas[int(q3[0]), int(q3[1])]==seg_colour).all()):
        pt = q3.copy()
        while((seg_canvas[int(pt[0]), int(pt[1])] == seg_colour).all()):
            pt+=vsteps
        pts[6] = pt
...
def get_body_control(lead_peaks, seg_canvas):
    return []

def get_control_points(lead_peaks, seg_canvas):
    # print(lead_peaks)
    control_arm_upper_l = get_limb_control(lead_peaks[5], lead_peaks[6], seg_canvas, np.array([0,255,255]))
    control_arm_lower_l = get_limb_control(lead_peaks[6], lead_peaks[7], seg_canvas, np.array([255,0,255]))
    control_arm_upper_r = get_limb_control(lead_peaks[2], lead_peaks[3], seg_canvas, np.array([0,127,255]))
    control_arm_lower_r = get_limb_control(lead_peaks[3], lead_peaks[4], seg_canvas, np.array([255,0,127]))
    control_leg_upper_l = get_limb_control(lead_peaks[11], lead_peaks[12], seg_canvas, np.array([0,255,0]))
    control_leg_lower_l = get_limb_control(lead_peaks[12], lead_peaks[13], seg_canvas, np.array([127,255,127]))
    control_leg_upper_r = get_limb_control(lead_peaks[8], lead_peaks[9], seg_canvas, np.array([255,0,0]))
    control_leg_lower_r = get_limb_control(lead_peaks[9], lead_peaks[10], seg_canvas, np.array([255,127,0]))
    control_body = get_body_control(lead_peaks, seg_canvas)
    controls = {"keypoints":lead_peaks,
        "control_arm_upper_l":control_arm_upper_l, 
        "control_arm_upper_r":control_arm_upper_r, 
        "control_arm_lower_l":control_arm_lower_l, 
        "control_arm_lower_r":control_arm_lower_r, 
        "control_leg_upper_l":control_leg_upper_l, 
        "control_leg_upper_r":control_leg_upper_r, 
        "control_leg_lower_l":control_leg_lower_l, 
        "control_leg_lower_r":control_leg_lower_r, 
...
def process (input_image, params, model, model_params):
    ...
    canvas = np.zeros(cv2.imread(input_image).shape, dtype=np.uint8)
    ...
    for i in range(18):
        if(len(all_peaks[i])>0):
            cv2.circle(canvas, all_peaks[i][0][0:2], 4, colors[i], thickness=-1)
            cv2.putText(canvas, str(i), all_peaks[i][0][0:2], cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0), 2)

    lead_peaks = -np.ones((18,2))
    for i in range(18):
        for j in range(len(all_peaks[i])):
            lead_peaks[i] = all_peaks[i][j][0:2]
    # print(lead_peaks)
    ...
    return canvas, lead_peaks, heatmap_avg, paf_avg, people, seg_avg

def controls(filename):
    args = parser.parse_args()
    ...
    if filename.endswith(".png") or filename.endswith(".jpg"):
        # print(filename)
        sk_canvas, lead_peaks, heatmap, paf, people, seg = process(filename, params, model, model_params)
        # cv2.imwrite(args.output_folder + '/sk_' + filename, sk_canvas)
        ...
        controls = get_control_points(lead_peaks, seg_canvas.transpose(1,0,2))
        # print(controls)
        return controls
...
def segs(filename):
    ...
    if filename.endswith(".png") or filename.endswith(".jpg"):
        ...
        _, _, _, mask = oriImg.split()
        seg_canvas = np.uint8(seg_canvas)
        mask = np.uint8(mask)
        mask = np.where(mask>50,mask,0)
        seg_masked = cv2.bitwise_and(seg_canvas, seg_canvas, mask=mask)
        # seg_masked = seg_masked.transpose(1,0,2)
        
        return lead_peaks, seg_masked
...
if __name__ == '__main__':
    ...
    for filename in os.listdir(args.input_folder):
        if filename.endswith(".png") or filename.endswith(".jpg"):
            ...
            sk_canvas, lead_peaks, heatmap, paf, people, seg = process(args.input_folder+'/'+filename, params, model_params)
            # cv2.imwrite(args.output_folder + '/sk_' + filename, sk_canvas)
            ...
            for j in range(8):
                if(len(controls["control_arm_upper_l"])>0):
                    cv2.circle(sk_canvas, (int(controls["control_arm_upper_l"][j][0]),int(controls["control_arm_upper_l"][j][1])), 4, (255,255,255), thickness=-1)
                if(len(controls["control_arm_upper_r"])>0):
                    cv2.circle(sk_canvas, (int(controls["control_arm_upper_r"][j][0]),int(controls["control_arm_upper_r"][j][1])), 4, (255,255,255), thickness=-1)
                if(len(controls["control_arm_lower_l"])>0):
                    cv2.circle(sk_canvas, (int(controls["control_arm_lower_l"][j][0]),int(controls["control_arm_lower_l"][j][1])), 4, (255,255,255), thickness=-1)
                if(len(controls["control_arm_lower_r"])>0):
                    cv2.circle(sk_canvas, (int(controls["control_arm_lower_r"][j][0]),int(controls["control_arm_lower_r"][j][1])), 4, (255,255,255), thickness=-1)
                if(len(controls["control_leg_upper_l"])>0):
                    cv2.circle(sk_canvas, (int(controls["control_leg_upper_l"][j][0]),int(controls["control_leg_upper_l"][j][1])), 4, (255,255,255), thickness=-1)
                if(len(controls["control_leg_upper_r"])>0):
                    cv2.circle(sk_canvas, (int(controls["control_leg_upper_r"][j][0]),int(controls["control_leg_upper_r"][j][1])), 4, (255,255,255), thickness=-1)
                if(len(controls["control_leg_lower_l"])>0):
                    cv2.circle(sk_canvas, (int(controls["control_leg_lower_l"][j][0]),int(controls["control_leg_lower_l"][j][1])), 4, (255,255,255), thickness=-1)
                if(len(controls["control_leg_lower_r"])>0):
                    cv2.circle(sk_canvas, (int(controls["control_leg_lower_r"][j][0]),int(controls["control_leg_lower_r"][j][1])), 4, (255,255,255), thickness=-1)

            # canvas = cv2.addWeighted(seg_canvas, 0.6, cur_canvas, 0.4, 0)
            canvas = cv2.addWeighted(seg_canvas, 0.4, sk_canvas, 0.6, 0)
            # canvas = seg_canvas
