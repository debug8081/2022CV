...
class Decoder(nn.Module):
    ...
    def forward(self, out2h, out3h, out4h, out5v, fback=None):
        if fback is not None:
            refine5      = F.interpolate(fback, size=out5v.size()[2:], mode='bilinear', align_corners=True)
            refine4      = F.interpolate(fback, size=out4h.size()[2:], mode='bilinear', align_corners=True)
            refine3      = F.interpolate(fback, size=out3h.size()[2:], mode='bilinear', align_corners=True)
            refine2      = F.interpolate(fback, size=out2h.size()[2:], mode='bilinear', align_corners=True)
            out5v        = out5v+refine5
...
class F3Net(nn.Module):
    ...
    def forward(self, x, shape=None):
        ...
        pred1 = F.interpolate(self.linearp1(pred1), size=shape, mode='bilinear', align_corners=True)
        pred2 = F.interpolate(self.linearp2(pred2), size=shape, mode='bilinear', align_corners=True)
